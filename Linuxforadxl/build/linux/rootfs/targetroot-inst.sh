...
setperm()
{
	...
	# updated/overwritten during make install
	chmod u+w ${1}
	if [ "$perm" ]
	then
		[ "$v" ] && echo "chmod ${perm} ${1}"
...
file_copy()
{
	...
	if [ -d "${src}" ]
	then
		[ "$v" ] && echo "CopyDir ${src} ${TARGETDIR}${dst}"
		(
			cd ${src} || return 1
			V=
			[ "$v" ] && V=v
			...
			find . -print | grep -E -v '/CVS|/\.svn' | ( cd ${TARGETDIR}${dst}; xargs chmod u+w )
		)
	else
		if [ -d ${dst} ]; then
			dstfile=${TARGETDIR}${dst}/`basename ${src}`
		else
			dstfile=${TARGETDIR}${dst}
		fi
		rm -f ${dstfile}
...
file_append()
{
	touch ${TARGETDIR}${dst} || return 1
	if [ -z "${pattern}" ] && grep -F "${src}" ${TARGETDIR}${dst} > /dev/null
	then
		[ "$v" ] && echo "File entry already installed."
	elif [ "${pattern}" ] && egrep "${pattern}" ${TARGETDIR}${dst} > /dev/null
	then
		[ "$v" ] && echo "File pattern already installed."
	else
		[ "$v" ] && echo "Installing entry into ${TARGETDIR}${dst}."
		echo "${src}" >> ${TARGETDIR}${dst} || return 1
	fi
	setperm ${TARGETDIR}${dst}
	return $?
}
...
{
	rm -f ${TARGETDIR}${dst}
	[ "$v" ] && echo "ln ${src} ${TARGETDIR}${dst}"
	ln ${TARGETDIR}${src} ${TARGETDIR}${dst}
	return $?
}
...
{
	rm -rf ${TARGETDIR}${dst}
	[ "$v" ] && echo "ln -s ${src} ${TARGETDIR}${dst}"
	ln -sf ${src} ${TARGETDIR}${dst}
	return $?
}
...
if [ "$mdir" -a ! -d "`dirname ${TARGETDIR}${dst}`/." ]
then
	mkdir -p "`dirname ${TARGETDIR}${dst}`/." || exit 1
fi
